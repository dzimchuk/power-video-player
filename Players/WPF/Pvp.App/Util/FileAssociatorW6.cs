...
namespace Pvp.App.Util
{
    ...
    {
        private Shell.IApplicationAssociationRegistration _pAppAssocReg;
        private const int S_OK = 0;
        private IDictionary<string, string> _dictProgIdApps;
        private IDictionary<string, string> _dictExtApps;

        public new static FileAssociator GetFileAssociator(string docTypePrefix, string appName)
        {
        ...
        private FileAssociatorW6(string docTypePrefix, string appName) : base(docTypePrefix, appName)
        {
            ...
            try
            {
                ...
                _pAppAssocReg = (Shell.IApplicationAssociationRegistration)comobj;
            }
            catch
            {
                if (comobj != null)
        ...
        {
            if (_pAppAssocReg != null)
            {
                bool bAssoc = false;
                int hr = _pAppAssocReg.QueryAppIsDefault(ext, Shell.ASSOCIATIONTYPE.AT_FILEEXTENSION, Shell.ASSOCIATIONLEVEL.AL_EFFECTIVE, _appName, out bAssoc);
        ...
        public override void Associate(string ext, bool bAssociate)
        {
            if (_pAppAssocReg != null)
            {
                if (bAssociate)
                {
                    ...
                    int hr = _pAppAssocReg.QueryCurrentDefault(ext, Shell.ASSOCIATIONTYPE.AT_FILEEXTENSION, Shell.ASSOCIATIONLEVEL.AL_EFFECTIVE, out prevProgId);
                    if (hr == S_OK)
                    {
                    ...
                    _pAppAssocReg.SetAppAsDefault(_appName, ext, Shell.ASSOCIATIONTYPE.AT_FILEEXTENSION);
                }
                else if (IsAssociated(ext))
                {
                    ...
                    if (prevApp != null)
                    {
                        _pAppAssocReg.SetAppAsDefault(prevApp, ext, Shell.ASSOCIATIONTYPE.AT_FILEEXTENSION);
        ...
        protected override void CleanUp()
        {
            if (_pAppAssocReg != null)
            {
                Marshal.FinalReleaseComObject(_pAppAssocReg);
                _pAppAssocReg = null;
            }
