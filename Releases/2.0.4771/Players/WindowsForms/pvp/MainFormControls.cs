...
namespace Pvp
{
    ...
    public class MainFormControls : MainFormFile
    {
        ...
        {
            if (ContainsFocus && mediaControl.GraphState != GraphState.Stopped &&
                mediaControl.GraphState != GraphState.Reset)
            {
        ...
        protected void PlayIt(string source, MediaSourceType CurrentlyPlaying)
        {
            ...
            if (mediaControl.BuildGraph(source, CurrentlyPlaying))
            {
                int volume;
                if (mediaControl.GetVolume(out volume))
                {
            ...
            engine_ModifyMenu(mediaControl, EventArgs.Empty); 
            UpdateButtons();
        }
        ...
        {
            GraphState state = mediaControl.GraphState;
            ...
            miRepeat.Checked = mediaControl.Repeat;
            miFullscreen.Checked = bFullscreen;

            if (miCloseMenu != null && contextMenu.MenuItems.Contains(miCloseMenu))
                miCloseMenu.Enabled = mediaControl.IsResumeDVDEnabled();
        }

        private void OnPlay(object sender, EventArgs e)
        {
            mediaControl.ResumeGraph();
            UpdateButtons();
        }

        private void OnPause(object sender, EventArgs e)
        {
            mediaControl.PauseGraph();
            UpdateButtons();
        }

        private void OnPause2(object sender, EventArgs e)
        {
            switch(mediaControl.GraphState)
            {
        ...
        {
            mediaControl.Repeat=!mediaControl.Repeat;
            UpdateButtons();
        }
        ...
        {
            GraphState state = mediaControl.GraphState;
            bool bSeekable = mediaControl.IsGraphSeekable;
            ...
            controlbar.btnRepeat.Pressed = mediaControl.Repeat;
        ...
        private void OnScrollVolumeBar(object sender, EventArgs e)
        {
            ...
            if (!bMute && bCanChangeVolume)
                mediaControl.SetVolume(nVolume);
        }
        ...
        private void OnScrollSeekBar(object sender, EventArgs e)
        {
            ...
            mediaControl.SetCurrentPosition(time);
        }
        ...
        {
            if (mediaControl.IsGraphSeekable)
            {
                long time=mediaControl.GetCurrentPosition();
        ...
        {
            mediaControl.SetCurrentPosition(0);
        }

        private void OnToEnd(object sender, EventArgs e)
        {
            long duration = mediaControl.GetDuration();
            mediaControl.SetCurrentPosition(duration-2500000);
            mediaControl.PauseGraph();
            UpdateButtons();
        }
        ...
        {
            nicon.Restore();
            mediaControl.VideoSize = VideoSize.SIZE50;
        }

        private void OnVideoSize100(object sender, EventArgs e)
        {
            nicon.Restore();
            mediaControl.VideoSize = VideoSize.SIZE100;
        }

        private void OnVideoSize200(object sender, EventArgs e)
        {
            nicon.Restore();
            mediaControl.VideoSize = VideoSize.SIZE200;
        }
        ...
        {
            MenuItemEx popup = (MenuItemEx)sender;
            AspectRatio ratio = mediaControl.AspectRatio;
        ...
        {
            mediaControl.AspectRatio = (AspectRatio)((MenuItemEx)sender).Tag;
        }

        private void OnRatePopup(object sender, EventArgs e)
        {
            MenuItemEx popup = (MenuItemEx)sender;
            double rate = mediaControl.Rate;
            foreach(MenuItemEx item in popup.MenuItems)
            {
                item.Enabled = mediaControl.IsGraphSeekable;
        ...
        {
            mediaControl.Rate = (double)((MenuItemEx)sender).Tag;
        }
        ...
        {
            if (mediaControl.FilterCount == 0)
                return;
            miFilters = new MenuItemEx(Resources.Resources.mi_filters);
            miFilters.Popup += new EventHandler(OnFiltersPopup);
            int nLast = mediaControl.FilterCount <= 15 ? mediaControl.FilterCount : 15;
            EventHandler eh = new EventHandler(OnFilterClick);
            for (int i=0; i<nLast; i++)
            {
                MenuItemEx item = new MenuItemEx(mediaControl.GetFilterName(i));
                item.Click += eh;
                item.Tag = i;
        ...
        private void OnFiltersPopup(object sender, EventArgs e)
        {
            MenuItemEx popup = (MenuItemEx)sender;
            foreach(MenuItemEx item in popup.MenuItems)
                item.Enabled = mediaControl.DisplayFilterPropPage(Handle, (int)item.Tag, false);
        }
        ...
        private void OnFilterClick(object sender, EventArgs e)
        {
            ...
            mediaControl.DisplayFilterPropPage(Handle, (int)item.Tag, true);
            bOkToHideCursor = true;
        }

        private void CreateAudioStreamsMenu(Menu popup, int nPos, bool bAppend)
        {
            int nStreams=mediaControl.AudioStreams;
            if (nStreams == 0)
                return;
            ...
            for (int i=0; i<nLast; i++)
            {
                MenuItemEx item = new MenuItemEx(mediaControl.GetAudioStreamName(i));
                item.Click += eh;
                item.Tag = i;
        ...
        private void OnAudioStreamsPopup(object sender, EventArgs e)
        {
            ...
            foreach(MenuItemEx item in popup.MenuItems)
            {
                ...
                item.Checked = mediaControl.CurrentAudioStream == (int)item.Tag;
            }
        }

        private void OnAudioStream(object sender, EventArgs e)
        {
            mediaControl.CurrentAudioStream = (int)((MenuItemEx)sender).Tag;
        }
        ...
        {
            int ulTitles=mediaControl.NumberOfTitles;
            if (ulTitles == 0)
                return;
            ...
            {
                ch_count=0;
                for (int ch=1; ch<=mediaControl.GetNumChapters(1); ch++)
                {
            ...
            else
            {
                ...
                for (int i=1; i<=ulTitles; i++)
                {
                    ...
                    ptitle_menu.Tag = i;
                    ch_count=0;
                    for (int ch=1; ch<=mediaControl.GetNumChapters(i); ch++)
                    {
                        MenuItemEx item = new MenuItemEx(String.Format(Resources.Resources.mi_chapter_format, ch));
                        item.Click += eh;
        ...
        {
            bool bEnable=(mediaControl.UOPS & VALID_UOP_FLAG.UOP_FLAG_Play_Chapter) == 0;
            MenuItemEx popup = (MenuItemEx)sender;
            int title = mediaControl.CurrentTitle;
            int chapter = mediaControl.CurrentChapter;
            foreach(MenuItemEx item in popup.MenuItems)
            {
                if (item.IsParent)
                {
        ...
        private void OnGoTo(object sender, EventArgs e)
        {
            ...
            mediaControl.GoTo(tc.title, tc.chapter);
        }

        private void CreateAnglesMenu(Menu popup, int nPos, bool bAppend)
        {
            int ulAngles=mediaControl.AnglesAvailable;
            if (ulAngles < 2)
                return;
        ...
        {
            bool bEnable = (mediaControl.UOPS & VALID_UOP_FLAG.UOP_FLAG_Select_Angle) == 0;
            MenuItemEx popup = (MenuItemEx)sender;
            foreach(MenuItemEx item in popup.MenuItems)
            {
                item.Checked = (int)item.Tag == mediaControl.CurrentAngle;
                item.Enabled = bEnable;
            }
        }
        ...
        {
            mediaControl.CurrentAngle = (int)((MenuItemEx)sender).Tag;
        }
        ...
        {
            VALID_UOP_FLAG UOPS = mediaControl.UOPS;
            MenuItemEx popup = (MenuItemEx)sender;
            foreach(MenuItemEx item in popup.MenuItems)
            {
        ...
        private void CreateDVDMenusLangMenu(Menu popup, int nPos, bool bAppend)
        {
            ...
            for (int i=0; i<nLast; i++)
            {
                MenuItemEx item = new MenuItemEx(mediaControl.GetMenuLangName(i));
                item.Click += eh;
                item.Tag = i;
        ...
        private void OnMenuLangPopup(object sender, EventArgs e)
        {
            MenuItemEx popup = (MenuItemEx)sender;
            foreach(MenuItemEx item in popup.MenuItems)
                item.Enabled = mediaControl.MenuLangCount > 1;
        }

        private void OnMenuLang(object sender, EventArgs e)
        {
            mediaControl.SetMenuLang((int)((MenuItemEx)sender).Tag);
        }

        private void CreateSubpictureMenu(Menu popup, int nPos, bool bAppend)
        {
            int nStreams=mediaControl.NumberOfSubpictureStreams;
            if (nStreams == 0)
                return;
            ...
            {
                item = new MenuItemEx(mediaControl.GetSubpictureStreamName(i));
                item.Click += eh;
                item.Tag = i;
                if (s_count==16)
                {
        ...
        private void OnSubtitlesPopup(object sender, EventArgs e)
        {
            ...
            foreach(MenuItemEx item in popup.MenuItems)
                if (item.Tag != null)
                {
                    if ((int)item.Tag == -1)
                    {
                        item.Checked = mediaControl.IsSubpictureEnabled();
                    ...
                    else
                    {
                        ...
                        bool bEnable = (UOPS & VALID_UOP_FLAG.UOP_FLAG_Select_SubPic_Stream)==0 && 
                            mediaControl.IsSubpictureStreamEnabled(tag);
                        item.Checked = tag == mediaControl.CurrentSubpictureStream;
                        item.Enabled = bEnable;
                    }
                }
        }
        ...
        {
            mediaControl.EnableSubpicture(!mediaControl.IsSubpictureEnabled());
        }

        private void OnSubpictureStream(object sender, EventArgs e)
        {
            mediaControl.CurrentSubpictureStream = (int)((MenuItemEx)sender).Tag;
        }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            DVD_DOMAIN domain;
            mediaControl.GetCurrentDomain(out domain);
        ...
        {
            if (mediaControl.GraphState == GraphState.Reset)
                return;

            ImageCreator imageCreator = new ImageCreator();
            try
            {
                mediaControl.GetCurrentImage(imageCreator);
